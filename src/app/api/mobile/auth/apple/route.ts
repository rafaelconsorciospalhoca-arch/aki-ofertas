...
const bodySchema = z.object({
  ...
  // Only present on the user's very first "Sign in with Apple" ever, per
  // Apple's own client SDK — never resend-able, so we take it when offered.
...
export async function POST(request: Request) {
  try {
    ...
    const profile = await verifyAppleIdentityToken(parsed.data.idToken)
    if (!profile) {
      return NextResponse.json({ ok: false, error: 'Não foi possível verificar o login com Apple.' }, { status: 401 })
    }
    if (!profile.email) {
      return NextResponse.json(
        { ok: false, error: 'Sua conta Apple não compartilhou um e-mail. Tente outra forma de entrar.' },
        { status: 400 },
      )
    }
    // User.email is a case-sensitive unique column; normalizing here keeps a
    // single account per address across Apple, Google, OTP and the site sign-up.
    const email = profile.email.trim().toLowerCase()

    const name =
      [parsed.data.fullName?.givenName, parsed.data.fullName?.familyName].filter(Boolean).join(' ').trim() ||
      email.split('@')[0]
    ...
    console.error('POST /api/mobile/auth/apple failed', err)
    return NextResponse.json({ ok: false, error: 'Erro interno. Tente novamente.' }, { status: 500 })
  }
}
